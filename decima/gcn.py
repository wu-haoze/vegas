...
class GraphCNN(object):
    def __init__(self, inputs, input_dim, hid_dims, output_dim,
        ...
        # message passing
        self.adj_mats = [tf.placeholder(
            tf.float32, [None, None]) for _ in range(self.max_depth)]
        self.adj_mats_sparse = [tf.sparse_placeholder(
            tf.float32, [None, None]) for _ in range(self.max_depth)]
        ...
        self.outputs_sparse = self.forward_sparse()
    ...
    def forward(self):
        ...
        x = self.inputs
        #print("GCN weights ", self.prep_weights[0].op.outputs[0], self.prep_bias[0].op.outputs[0], self.proc_weights[0].op, self.proc_bias[0].op, self.agg_weights[0].op, self.agg_bias[0].op)
        # raise x into higher dimension
        #print("X ", self.prep_weights[0].shape, self.prep_weights[1].shape)
        #for weight in self.prep_weights:
        #    print("prep weight shape ",weight.shape)
        #for weight in self.proc_weights:
        #    print("proc weight shape ",weight.shape)
        #for weight in self.agg_weights:
        #    print("agg weight shape ",weight.shape)

        #for bias in self.prep_bias:
        #    print("prep bias shape ",bias.shape)
        #for bias in self.proc_bias:
        #    print("proc bias shape ",bias.shape)
        #for bias in self.agg_bias:
        #    print("agg bias shape ",bias.shape)
            
            
        #for mat in self.adj_mats:
        #    print("adj mat ", mat)
        ...
        for d in range(self.max_depth):
            ...
            for l in range(len(self.proc_weights)):
                ...
                y += self.proc_bias[l]
                y = self.act_fn(y)
            #print("Y ", y, self.adj_mats[d])
            # message passing
            y = tf.matmul(self.adj_mats[d], y)
    ...
    def forward_sparse(self):
        ...
        for d in range(self.max_depth):
            ...
            # message passing
            y = tf.sparse_tensor_dense_matmul(self.adj_mats_sparse[d], y)
