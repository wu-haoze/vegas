...
def get_dag_durations(job_dags):
...
for exp in range(args.num_exp):
    ...
    for scheme in args.test_schemes:
        ...
        durations[scheme] = get_dag_durations(env.finished_job_dags)

    # plot CDF - job durations

    fig = plt.figure()
    ax = fig.add_subplot(111)

    for scheme in args.test_schemes:
        x, y = compute_CDF(durations[scheme])
        ax.plot(x, y)

    plt.xlabel('Job duration (milliseconds)')
    plt.ylabel('CDF')
    plt.legend(args.test_schemes)
    plt.yticks(np.arange(1e5, 1.1e6, 1e5))
    plt.yticks(np.arange(0., 1.05, .1))
    plt.grid()
    fig.savefig(args.result_folder + 'job_durations_cdf.png')
