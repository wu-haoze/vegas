...
def generate_same_tpch_jobs(np_random, timeline, wall_time, jobIndexToRepeat):
    job_dags = OrderedSet()
    t = 0

    for i in range(args.num_init_dags)[:-1]:
        # generate query
        query_idx = str(np_random.randint(args.tpch_num) + 1)
        query_size = args.tpch_size[np_random.randint(len(args.tpch_size))]
...
def generate_jobs(np_random, timeline, wall_time):
    if args.query_type == 'tpch':
        if args.repeat_job == -1:
            job_dags = generate_tpch_jobs(np_random, timeline, wall_time)
        else:
            job_dags = generate_same_tpch_jobs(np_random, timeline, wall_time, args.repeat_job)
