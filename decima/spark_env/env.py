...
class Environment(object):
    ...
    def add_job(self, job_dag, arrived=False):
        if arrived:
    ...
    def step(self, next_node, limit):
        ...
        if done:
            assert self.wall_time.curr_time >= self.max_time or \
                   len(self.job_dags) == 0 or \
                   all(['adversarial' in job_dag.name for job_dag in self.job_dags])

        return self.observe(), reward, done

    def remove_job(self, job_dag, unfinished=False):
