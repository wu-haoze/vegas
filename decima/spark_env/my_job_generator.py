"""
Used to create one job with 1 parent stage and 25 children, followed by
25 other jobs with 1 stage each. In the large job, the parent stage has
enough tasks (50) to use all the executors, whereas the other stages have
one task each. In stages of the small jobs contain 1 task each.
"""
...
def get_large_job(np_random, wall_time, jid, n_child=5, n_tasks=50):
    """
    Returns a large job with one parent node and n_child children
    nodes. The parent will have n_tasks tasks, while each child will
    have one task only.
    """
    adj_mat = np.zeros((n_child+1, n_child+1))
    adj_mat[0][1:] = 1

    # task durations
    parent_task_duration = {"first_wave": {N_TRAIN_EXECUTORS: [sample_task_time(True, np_random)]},
                            "rest_wave": {N_TRAIN_EXECUTORS: [sample_task_time(False, np_random) for i in range(n_tasks-1)]},
                            "fresh_durations": {N_TRAIN_EXECUTORS: [sample_task_time(True, np_random)]}}
    task_durations = [parent_task_duration]
    for i in range(n_child):
        child_task_duration = {"first_wave": {N_TRAIN_EXECUTORS: [sample_task_time(True, np_random)]},
                               "rest_wave": {N_TRAIN_EXECUTORS: []},
                               "fresh_durations": {N_TRAIN_EXECUTORS: [sample_task_time(True, np_random)]}}
        task_durations.append(child_task_duration)
...
def get_small_job(np_random, wall_time, jid):
    """
    Returns a small job with a single stage and a single task.
    """
...
def generate_jobs(np_random, timeline, wall_time, n_large_dags=1, n_small_dags=25):

    job_dags = OrderedSet()
    t = 0

    # add large dags
    for jid in range(n_large_dags):
        job_dag = get_large_job(np_random, wall_time, f"large_job_{jid}")
