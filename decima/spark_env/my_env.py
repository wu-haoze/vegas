"""
Tweaked variant of the original env to use our job generator
"""
...
class Environment(object):
    def __init__(self, generate_jobs):
        ...
        print(f"Number of executors: {len(self.executors)}")

        # free executors
        self.free_executors = FreeExecutors(self.executors)
        print(f"Number of free executors: {len(self.free_executors.free_executors)}")
        ...
        if executor.node is not None and not executor.node.no_more_tasks:
            ...
            if frontier_changed:
                ...
                if len(self.exec_commit[executor.node]) > 0:
                    ...
                    print("calling schedule() from \"assign_executor\" #1")
                    self.schedule()
                else:
                ...
                if len(self.exec_commit[executor.node]) > 0:
                    # directly fulfill the commitment
                    print("calling schedule() from \"assign_executor\" #2")
                    self.schedule()
                else:
    ...
    def get_frontier_nodes(self):
        ...
        for job_dag in self.job_dags:
            for node in job_dag.nodes:
                # if node.idx==0 and node.job_dag.name=="large_job_0":
                #     print(f"Is node 0 of large_job_0 saturated? {self.saturated(node)}")
                #     print(f"Is node 0 of large_job_0 in node_selected? {node in self.node_selected}")
            ...
            if node is None:
                ...
                if executor.job_dag is not None and \
                   any([not n.no_more_tasks for n in \
                    ...
                    self.free_executors.add(executor.job_dag, executor)
                    print(f"{itr}: node is None, setting executor {executor.idx} as idle in job {executor.job_dag.name}")
                else:
                    # no where to assign, put executor in null pool
                    self.free_executors.add(None, executor)
                    print(f"{itr}: node is None, assigning executor {executor.idx} to null pool")
                ...
                if executor.job_dag == node.job_dag:
                    ...
                    print(f"{itr} - scheduling node {node.idx} of job {node.job_dag.name} to (local) executor {executor.idx}")
                    if node in node.job_dag.frontier_nodes:
                        # node is immediately runnable
                        task = node.schedule(executor)
                        self.timeline.push(task.finish_time, task)
                        print("-> executor can run immediately")
                    else:
                        # put executor back in the free pool
                        self.free_executors.add(executor.job_dag, executor)
                        print("executor can run immediately")
                        print("-> executor cannot run immediately -- set to idle for the moment")

                else:
                    # need to move executor
                    print(f"{itr} - scheduling node {node.idx} of job {node.job_dag.name} to (remote) executor {executor.idx}")
                    self.timeline.push(
                        self.wall_time.curr_time + args.moving_delay, executor)
                    # keep track of moving executors
                    self.moving_executors.add(executor, node)
                ...
                print(f"{itr} - attempted to schedule saturated node {node.idx} of job {node.job_dag.name} to executor {executor.idx}; running backup logic")
    ...
    def step(self, next_node, limit):
        ...
        if next_node is not None:
            ...
            use_exec = limit
        assert use_exec > 0
        print(f"assigned {use_exec} additional executor(s) to node")
        self.exec_commit.add(source, next_node, use_exec)
        if next_node is not  None:
            total_exec = len(next_node.executors) + \
                         self.exec_commit.node_commit[next_node] + \
                         self.moving_executors.count(next_node)
            print(f"overall number of executors assigned to node: {total_exec}")
        ...
        if self.num_source_exec == 0:
            ...
            print("calling schedule() from \"step\"")
            self.schedule()
        ...
        while len(self.timeline) > 0 and self.num_source_exec == 0:
            ...
            print(f"current time: {self.wall_time.curr_time:0.2f}")
            ...
            if isinstance(obj, Task):  # task completion event
                ...
                print(f"Event: task {finished_task.idx} of node {finished_task.node.idx} of job {finished_task.node.job_dag.name} done running on executor {finished_task.executor.idx}")
                ...
                print(f"num_source_exec before call to assign executor {self.num_source_exec}")
                self.assign_executor(finished_task.executor, frontier_changed)
                print(f"num_source_exec after call to assign executor {self.num_source_exec}")
                ...
                print(f"Event: executor {executor.idx} done moving to node {node.idx} of job {node.job_dag.name}")
    ...
    def reset(self, max_time=np.inf):
        ...
        self.job_dags = self.generate_jobs(
